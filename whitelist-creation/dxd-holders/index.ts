...
const ARBITRUM_PURE_HOLDERS_CACHE_LOCATION = `${__dirname}/cache/arbitrum-holders.json`;
...
const getMainnetDxdVestingContractAddresses = async () => {
    ...
    const startingBlock = 10699672; // vesting factory deployment block
    const endingBlock = XMAS_MAINNET_SNAPSHOT_BLOCK;
    let lastAnalyzedBlock = startingBlock;
    while (lastAnalyzedBlock < endingBlock) {
        const toBlock = lastAnalyzedBlock + 10000;
        logInPlace(
            `getting vesting contracts creations: ${(
                ((lastAnalyzedBlock - startingBlock) /
...
export const getXmasDxdHoldersBalanceMap = async (): Promise<{
    ...
    if (!pureXDaiHolders || Object.keys(pureXDaiHolders).length === 0) {
        pureXDaiHolders = await getDxdTokenHoldersWithBalances(
            XDAI_PROVIDER,
            DXD_XDAI_ADDRESS,
            15040609, // dxd token proxy deployment block
            XMAS_XDAI_SNAPSHOT_BLOCK
        );
    ...
    if (!pureMainnetHolders || Object.keys(pureMainnetHolders).length === 0) {
        pureMainnetHolders = await getDxdTokenHoldersWithBalances(
            MAINNET_PROVIDER,
            DXD_MAINNET_ADDRESS,
            10012634, // dxd token deployment block
            XMAS_MAINNET_SNAPSHOT_BLOCK
        );
        saveBalanceMapCache(
            pureMainnetHolders,
    ...
    if (!pureArbitrumHolders || Object.keys(pureArbitrumHolders).length === 0) {
        pureArbitrumHolders = await getDxdTokenHoldersWithBalances(
            ARBITRUM_PROVIDER,
            DXD_ARBITRUM_ADDRESS,
            6120, // dxd token deployment block
            XMAS_ARBITRUM_SNAPSHOT_BLOCK
        );
        saveBalanceMapCache(
            pureArbitrumHolders,
